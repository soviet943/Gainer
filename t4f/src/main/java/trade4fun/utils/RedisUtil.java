...
public class RedisUtil<T> implements InitializingBean {
     ...
     * 根据key设置数组对象
     */
    public long setObject(String key, Class clazz, Object... values) {
    ...
    /**
     * 获取列表中所有数据,并将字符串解析成jsonObject形式
     *
     * @param key
     * @return
     */
    public List<T> lall(String key, Class clazz) {
        ...
        try {
            jedis = jedisPool.getResource();
            // 0表示第一个元素，-1表示最后一个元素。即选取所有键对应数组内的所有元素
     ...
     * 根据数据库编号取值赋值
     */
    public String put(String key, String value, int indexOfDatabase) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexOfDatabase);
            return jedis.set(key, value);
        } finally {
            if (jedis != null) {
                jedis.close();
    ...
    public String get(String key, int indexOfDatabase) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexOfDatabase);
